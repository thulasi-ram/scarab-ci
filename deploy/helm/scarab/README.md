...
| `workspace.blobAuthz` | `""` = `log` | blob-read authorization mode (`SCARAB_DEPOT_BLOB_AUTHZ`, ticket 52ef3aa): `off`/`log`/`enforce`; flip to `enforce` after `scarab_depot_blob_authz_would_deny_total` stays zero over a representative window |
