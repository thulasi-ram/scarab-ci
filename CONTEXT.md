...
| **Superseded** | Terminal outcome of an Attempt that was **cut short while running because a human reran an upstream step** (restart-step re-arms in-flight descendants, `scheduler.rs`): its input was being replaced, so it could not honestly finish, and a fresh Attempt takes its place. Distinct from **Cancelled** — a *deliberate* stop of the Run/Step (Cancel, `cancel_run_request`) with **no** replacement coming — and from **Failed** (ran and errored). A superseded Attempt's evidence is retained like any other. |
| **Auto-retry** | The **engine** re-executing a not-yet-succeeded StepRun within its retry budget (ADR-0047). Machine-initiated, never on a succeeded or in-progress step; adds an Attempt but does **not** fork the Run (stays within the current Take). The only "retry" — there is no separate *manual retry* concept. |
| **Rerun** | The **human** re-execution action: re-running a **terminal** step — failed *or* succeeded — which **forks the Run into a new Take** (a new history row) and cascades to descendants. The engine operation is `restart-step`. The single human re-execution control (avoids a separate retry vs rerun split in the UI). _Avoid_: "manual retry", "restart" (user-facing). |
