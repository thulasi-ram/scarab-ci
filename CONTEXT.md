...
### 4.2 Data plane (eight distinct concepts — never conflate)
...
| **Change set** | one Attempt | derived; folded into a Snapshot, then discarded | the **exact** set of paths an Attempt wrote, known rather than inferred (ADR-0062: the `overlayfs` upper layer of its **Workspace Export**). What the drain hashes, and the reason a drain need not re-read an unchanged tree. Where no Export exists it is **approximated** by comparing each file's `(size, mtime)` against the input manifest — correct-but-conservative, and the one place in the data path where a wrong answer is possible rather than merely slow. |
