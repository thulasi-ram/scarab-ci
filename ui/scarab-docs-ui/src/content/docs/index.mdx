...
![The Scarab dashboard — an action inbox of runs suspended on a gate, recently-visited repos as pass/fail status cards, and the recency-sorted repo list](../../assets/dashboard.jpg)
...
<CardGrid>
  ...
  <Card title="Keyless identity" icon="seti:lock">
    ...
    cloud. Forge-native by design, with GitHub and Forgejo adapters implemented — see Status.
  </Card>
...
![A Scarab run — the step DAG, live logs, and an event timeline showing a retried step and the run suspended on a manual deploy gate](../../assets/run-detail.jpg)
...
that backend baggage on purpose; the pace is what a small AI-first team can now hold.
:::
...
:::note[Status — honest]
A proven core, with real forge and Kubernetes I/O wired in. **Proven against real Postgres:** the
durable engine — crash/resume, exactly-once, restart-a-step, gates, content-addressed workspace,
`invoke`, secrets, OIDC issuer (the workspace suite passes locally). **Implemented:** the GitHub
adapter (webhook ingest + commit-status posting) and a Forgejo adapter for multi-forge support;
live-Kubernetes Pod execution (`scarab-executor-k8s`, with cluster tests); the results-egress
sidecar image (`docker/sidecar/`); and CI that runs the Rust suite on push
(`.github/workflows/ci.yml`). None of it is yet hardened at scale — read a proven state machine
with its edges freshly wired, not a battle-tested product.
:::
