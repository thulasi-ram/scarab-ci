...
self-service (root inside the caps-dropped, unprivileged sandbox does not escape it);
...
- **`invoke: .scarab/lib/...`** — compile-time reuse (see [Reuse — `invoke`](#reuse--invoke)).
...
under the same name (a fork-PR run is locked out and receives none).
