...
use scarab_workspace_client::{DrainPostOutcome, DrainRecord, MemoCas, WorkspaceClient};
...
/// Ticket e140121: a **Depot restart mid-drain** is survivable by re-driving
/// the whole leg from scratch — the design's "session-lost is NOT
/// client-resumable and does not need to be".
///
/// Restart-equivalence without process murder (design amendment 6): a
/// replica's PackSessions are per-replica and in-memory, and its uncommitted
/// multipart bytes publish nothing — so "the Depot restarted" IS, at this
/// suite's grain, "the drain's early durable PUTs went into router A's open
/// tail pack (never committed to the shared index), and the re-drive runs
/// against router B over the SAME Postgres + cold store with no memory of A".
///
/// Asserted:
/// - the re-driven `/have` RE-reports the lost bytes (a blob A already
///   received uploads AGAIN through B — dedup keyed on the shared pack
///   index, which never saw A's uncommitted tail, not on anyone's warm);
/// - the record POST through B validates (durable-presence gate over the
///   shared index) and reads back through Browse;
/// - a duplicate POST — the lost-2xx shape — answers 409 and the client maps
///   it to `AlreadyPosted` (success): exit-code honesty for a drain whose
///   record is already deposited.
///
/// Mutations killed: `/have` dedup keying on warm-or-anything-replica-local
/// (the lost blob would be skipped and the record 422s forever), and 409
/// reverting to an error (a lost 2xx then re-drives a COMPLETED drain into
/// exit 12 laps).
#[tokio::test]
async fn a_depot_restart_mid_drain_re_drives_from_scratch_and_a_lost_2xx_is_posted() {
    let Some(a) = Harness::start().await else { return };
    let ws = tempfile::tempdir().unwrap();
    build_workspace(ws.path());

    // Partway against A: a durable fenced PUT of one of the workspace's OWN
    // blobs ("hello world" = plain.txt) lands in A's open in-memory tail pack
    // — appended to the shared write ledger, but NOT in the shared pack index
    // (no linger in this harness: nothing seals but a record POST, which
    // never comes on A).
    let helper_a = a.fence_client();
    scarab_storage::Cas::put_blob(&helper_a, b"hello world")
        .await
        .expect("the pre-restart durable PUT");

    // The restart: router B, same Postgres, same cold store, fresh warm, and
    // no memory of A's session.
    let b = a.replica().await;
    let helper_b = b.fence_client();
    let report = helper_b
        .drain_ingest_report(ws.path().to_str().unwrap(), &[], &[])
        .await
        .expect("the re-driven drain ingest through B");
    // The lost byte re-uploads: every distinct blob of the workspace misses
    // the shared pack index — INCLUDING plain.txt's, which A had already
    // received. `have_hits` counting it as a hit is exactly the mutation
    // that would 422 the record below forever.
    assert_eq!(
        report.have_hits, 0,
        "nothing A staged may count as durable-present after the restart"
    );
    assert_eq!(
        report.blobs_uploaded, 4,
        "all four distinct blobs (incl. the one A had) re-upload through B"
    );
    ...
    assert_eq!(
        helper_b
            .post_drain_record_classified(&rec)
            .await
            .expect("the re-driven record POST"),
        DrainPostOutcome::Posted,
        "the durable-presence gate must accept the re-driven closure"
    );

    // Visible through Browse (either replica — the record is a Postgres row).
    let got = b
        .browse_client()
        .drain_record("run-1", "build", "a1")
        .await
        .expect("record GET")
        .expect("the re-driven record must exist");
    assert_eq!(got, rec);

    // The lost-2xx shape: the identical POST again answers 409, and the
    // client reports it as success-already-standing, not an error.
    assert_eq!(
        helper_b
            .post_drain_record_classified(&rec)
            .await
            .expect("a duplicate POST is not a transport error"),
        DrainPostOutcome::AlreadyPosted,
        "409 under this fence is OUR record — exit-code honesty (e140121 F5)"
    );
    // And the plain wrapper agrees (the error-record best-effort path).
    helper_a
        .post_drain_record(&rec)
        .await
        .expect("post_drain_record maps AlreadyPosted to Ok");
}
