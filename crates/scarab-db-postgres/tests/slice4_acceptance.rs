...
use scarab_engine::{
    release_gate, ConcurrencyPolicy, Db, RunId, RunStatus, Scheduler, StepId, StepSpec, StepStatus,
    Timestamp,
};
...
async fn prod_group_serializes_with_gate_resume_across_restart() {
    ...
    db.set_run_concurrency(&r1, "deploy-prod", ConcurrencyPolicy::Queue)
        .await
        .unwrap();
    let gate = StepId("approve".into());
    db.create_step_run(&r1, &gate, None, &[], Timestamp(1))
        .await
        .unwrap();
    ...
    db.set_run_concurrency(&r2, "deploy-prod", ConcurrencyPolicy::Queue)
        .await
        .unwrap();
    db.create_step_run(
        &r2,
    ...
    assert_eq!(
        ...
        RunStatus::Suspended,
        "run1 suspended at the gate"
    );
    assert_eq!(
        status(&db, "run-2").await,
        RunStatus::Pending,
        "run2 serialized behind the prod slot"
    );
    ...
    assert_eq!(
        status(&db, "run-2").await,
        RunStatus::Succeeded,
        "run2 ran once run1 freed the slot"
    );
