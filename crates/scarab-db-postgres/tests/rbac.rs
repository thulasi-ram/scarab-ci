...
fn project(org: &str, name: &str) -> Scope {
    Scope::Project { org: org.into(), name: name.into() }
}

fn binding(subject: &str, scope: Scope, role: Role) -> Binding {
    Binding { subject: subject.into(), scope, role }
}
...
async fn scoped_roles_inherit_and_origins_respect_authority() {
    ...
    db.grant(&binding("alice", Scope::Org("acme".into()), Role::Member), BindingOrigin::Native)
        .await
        .unwrap();
    ...
    db.grant(&binding("alice", app.clone(), Role::Admin), BindingOrigin::Native)
        .await
        .unwrap();
    ...
    db.grant(&binding("bob", app.clone(), Role::Viewer), BindingOrigin::Native)
        .await
        .unwrap();
    assert_eq!(db.role_of("bob", &app).await.unwrap(), Some(Role::Viewer));
