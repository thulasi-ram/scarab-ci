...
async fn drive_to_gate(pg: &Arc<PostgresDb>, run: &RunId) {
    ...
    let spec = StepSpec { image: "busybox".into(), command: vec!["true".into()], env: vec![], secrets: vec![], run_as_root: false, add_capabilities: vec![], privileged: false, timeout_seconds: None, workspace_inputs: vec![] };
