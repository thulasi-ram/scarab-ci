...
another Pipeline — the recursion primitive), `service` (sidecar), and **`gate`** — a
**durable suspend** point (human approval / timer / external event). Gate is where the wedge
pays off: suspend for seconds or weeks at ~zero cost because state lives in Postgres.
