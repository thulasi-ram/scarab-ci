...
| **Widen at DISPATCH, not only on a human rerun** (ADR-0061 s5) | ADR-0061 s5 | Widening over expired Workspace Snapshots is wired into the human rerun/retry path (`rerun_step_widened` / `retry_step_widened`) — the path that has an affordance to relabel. A step promoted `Pending→Ready` whose upstream snapshot vanished *without* anyone pressing rerun still fails via a backstop — since ADR-0061 s3-feed that is the **fetcher** exiting non-zero from inside the Pod, which `pod_state` classifies as `Infra { never_started: true }`; the control-plane `DriveErr::InputMissing` that used to produce the identical verdict is deleted (tombstone: `crates/scarab-executor-k8s/src/lib.rs:1180`). An auto-retry then re-fails it until the infra budget dead-letters the run. The natural fix is the same `plan_rerun` call in the admission promotion, behind `Scheduler::with_snapshots(…)` (a builder, so no call-site churn) — deferred because it puts one object-store existence check per consumed input on the hot promotion path, and the case it covers is a sweep racing a live run, which the grace window already makes rare. Build it when a real run dead-letters that way. |
